...
async function main() {
  ...
  const existringPsychologyMethods = await prisma.psychologyMethod.findMany();
  const existingPsychotherapyMethods = await prisma.psychotherapyMethod.findMany();
  const therapiesToCreate = therapies.filter(
    therapy => !existingTherapies.find(existing => existing.type === therapy.type),
  );
  const psychologyMethodsToCreate = psychologyMethods.filter(
    method => !existringPsychologyMethods.find(existing => existing.title === method.title),
  );
  const psychotherapyMethodsToCreate = psychotherapyMethods.filter(
    method => !existingPsychotherapyMethods.find(existing => existing.title === method.title),
  );
  await prisma.$transaction(async trx => {
    await trx.therapy.createMany({
      data: therapiesToCreate,
    });
    await trx.psychologyMethod.createMany({ data: psychologyMethodsToCreate });
    await trx.psychotherapyMethod.createMany({ data: psychotherapyMethodsToCreate });
  });
}
