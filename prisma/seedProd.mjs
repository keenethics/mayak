...
const clientCategories = ['сім’ї військових', 'військові', 'ЛГБТ+', 'супровід у психіатрії', 'ВІЛ - інфіковані'].map(
  title => ({ title }),
);
...
async function main() {
  ...
  await createIfNotExist(prisma.clientCategory, clientCategories, ({ title }) => ({ title }));
}
